...
const add_preferences_button = document.getElementById("add-preferences-button");


function listenerButtonHandler() {
    window.location.href = 'ListenerPage.html';
}
...
function removePageButtonHandler() {
    window.location.href = 'removeSongs.html';
}

function addPreferencesButtonHandler() {
    window.location.href = 'addSongs.html';
}
...
add_preferences_button.addEventListener("click", addPreferencesButtonHandler);

const songs_search_bar = document.getElementById("song-search-bar");
songs_search_bar.addEventListener("keydown", function(event) {
    if (event.key === "Enter") {
        //user input from search bar
        const searchTerm = event.target.value.toLowerCase();;
        //const result = findSongByTitle(searchTerm);
        const result = findSongs(searchTerm);
        console.log(result);
        
        //unorsdered list on add song and remove song page
        ...
        result.forEach(result => {
            ...
            label.textContent = result.title + " - " + result.artist;
...
//Rolling Stone's list of 500 greatest songs of all time... debatable. 
const songs = [{
    "rank": 1,
